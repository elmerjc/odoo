...
class sale_order(osv.osv):
    ...
    _columns = {
        ...
        'order_line': fields.one2many('sale.order.line', 'order_id', 'Order Lines', readonly=True, states={'draft': [('readonly', False)], 'sent': [('readonly', False)], 'progress': [('readonly', False)], 'manual': [('readonly', False)], 'done': [('readonly', False)]}, copy=True),
...
class sale_order_line(osv.osv):
    ...
    _columns = {
        ...
        'price_unit': fields.float('Unit Price', required=True, digits_compute= dp.get_precision('Product Price'), readonly=True, states={'draft': [('readonly', False)], 'done': [('readonly', False)], 'confirmed': [('readonly', False)]}),
        ...
        'product_uom_qty': fields.float('Quantity', digits_compute= dp.get_precision('Product UoS'), required=True, readonly=True, states={'draft': [('readonly', False)], 'done': [('readonly', False)], 'confirmed': [('readonly', False)]}),
